<!--<div class="detallecompras index">-->
<div class="">
	<h2><?php echo __('Detallecompras'); ?></h2>
	<table cellpadding="0" cellspacing="0">
	<tr>
			<th><?php echo $this->Paginator->sort('id'); ?></th>
			<th><?php echo $this->Paginator->sort('compra_id'); ?></th>
			<th><?php echo $this->Paginator->sort('producto_id'); ?></th>
			<th><?php echo $this->Paginator->sort('precioproducto'); ?></th>
			<th><?php echo $this->Paginator->sort('cantidad'); ?></th>
			<th class="actions"><?php echo __('Actions'); ?></th>
	</tr>
	<?php foreach ($detallecompras as $detallecompra): ?>
	<tr>
		<td><?php echo h($detallecompra['Detallecompra']['id']); ?>&nbsp;</td>
		<td>
			<?php echo $this->Html->link($detallecompra['Compra']['id'], array('controller' => 'compras', 'action' => 'view', $detallecompra['Compra']['id'])); ?>
		</td>
		<td>
			<?php echo $this->Html->link($detallecompra['Producto']['id'], array('controller' => 'productos', 'action' => 'view', $detallecompra['Producto']['id'])); ?>
		</td>
		<td><?php echo h($detallecompra['Detallecompra']['precioproducto']); ?>&nbsp;</td>
		<td><?php echo h($detallecompra['Detallecompra']['cantidad']); ?>&nbsp;</td>
		<td class="actions">
			<?php echo $this->Html->link(__('View'), array('action' => 'view', $detallecompra['Detallecompra']['id'])); ?>
			<?php echo $this->Html->link(__('Edit'), array('action' => 'edit', $detallecompra['Detallecompra']['id'])); ?>
			<?php echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $detallecompra['Detallecompra']['id']), null, __('Are you sure you want to delete # %s?', $detallecompra['Detallecompra']['id'])); ?>
		</td>
	</tr>
<?php endforeach; ?>
	</table>
	<p>
	<?php
	echo $this->Paginator->counter(array(
	'format' => __('Page {:page} of {:pages}, showing {:current} records out of {:count} total, starting on record {:start}, ending on {:end}')
	));
	?>	</p>
	<div class="paging">
	<?php
		echo $this->Paginator->prev('< ' . __('previous'), array(), null, array('class' => 'prev disabled'));
		echo $this->Paginator->numbers(array('separator' => ''));
		echo $this->Paginator->next(__('next') . ' >', array(), null, array('class' => 'next disabled'));
	?>
	</div>
</div>
<!--<div class="actions">
	<h3><?php echo __('Actions'); ?></h3>
	<ul>
		<li><?php echo $this->Html->link(__('New Detallecompra'), array('action' => 'add')); ?></li>
		<li><?php echo $this->Html->link(__('List Compras'), array('controller' => 'compras', 'action' => 'index')); ?> </li>
		<li><?php echo $this->Html->link(__('New Compra'), array('controller' => 'compras', 'action' => 'add')); ?> </li>
		<li><?php echo $this->Html->link(__('List Productos'), array('controller' => 'productos', 'action' => 'index')); ?> </li>
		<li><?php echo $this->Html->link(__('New Producto'), array('controller' => 'productos', 'action' => 'add')); ?> </li>
	</ul>
</div>-->
